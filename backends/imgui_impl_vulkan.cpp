...
//  [x] Platform: Multi-viewport / platform windows. With issues (flickering when creating a new
//  viewport).
// Missing features:
...
//
//  Eddie: Add support for dynamic rendering.
...
bool ImGui_ImplVulkan_CreateDeviceObjects(
    std::optional<VkPipelineRenderingCreateInfo> render_create_info);
                                     ...
                                     VkPipeline pipeline)
{
    ...
    // Allocate array to store enough vertex/index buffers. Each unique viewport gets its own
    // storage.
    ...
    std::optional<VkPipelineRenderingCreateInfo> render_create_info)
{
    ...
    if (render_create_info.has_value())
    {
        info.pNext = &render_create_info.value();
    }
    ...
    std::optional<VkPipelineRenderingCreateInfo> render_create_info)
{
    ...
    ImGui_ImplVulkan_CreatePipeline(v->Device, v->Allocator, v->PipelineCache, bd->RenderPass,
                                    v->MSAASamples, &bd->Pipeline, bd->Subpass, render_create_info);
                           ...
                           std::optional<VkPipelineRenderingCreateInfo> render_create_info)
{
    ...
    // Allow render_pass to be null, which means we are using Vulkan 1.3's dynamic rendering
    IM_ASSERT(render_pass != VK_NULL_HANDLE || render_create_info.has_value());

    bd->VulkanInitInfo = *info;
    bd->RenderPass     = render_pass;
    bd->Subpass        = info->Subpass;

    ImGui_ImplVulkan_CreateDeviceObjects(render_create_info);
...
static void ImGui_ImplVulkan_RenderWindow(ImGuiViewport *viewport, void *)
{
    ...
    ImGui_ImplVulkanH_FrameSemaphores *fsd = &wd->FrameSemaphores[wd->SemaphoreIndex];
    {
        {
            err =
                vkAcquireNextImageKHR(v->Device, wd->Swapchain, UINT64_MAX,
                                      fsd->ImageAcquiredSemaphore, VK_NULL_HANDLE, &wd->FrameIndex);
            check_vk_result(err);
        ...
        }
        {
            ...
            info.pClearValues =
                (viewport->Flags & ImGuiViewportFlags_NoRendererClear) ? NULL : &wd->ClearValue;
...
static void ImGui_ImplVulkan_SwapBuffers(ImGuiViewport *viewport, void *)
{
    ...
    if (err == VK_ERROR_OUT_OF_DATE_KHR || err == VK_SUBOPTIMAL_KHR)
        ImGui_ImplVulkanH_CreateOrResizeWindow(
            v->Instance, v->PhysicalDevice, v->Device, &vd->Window, v->QueueFamily, v->Allocator,
